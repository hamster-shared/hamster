...
use sp_hamster::p_provider::ResourceStatus;
use sp_hamster::p_resource_order::{AgreementStatus, OrderStatus};
use sp_core::Bytes;
...
fn it_works_for_order_exec() {
	new_test_order().execute_with(|| {
		...
		let staking_info = sp_hamster::p_market::StakingAmount {
		...
		let market_staking = sp_hamster::p_market::TotalStakingAmount {
...
fn it_works_for_health_check() {
	new_test_health_check().execute_with(|| {
		...
		let provider_amount = sp_hamster::p_market::StakingAmount {
			amount: 800_000_000_000_000,
			lock_amount: 0,
			active_amount: 800_000_000_000_000,
		};

		let client_amount = sp_hamster::p_market::StakingAmount {
			amount: 1000_000_000_000_000,
			lock_amount: 0,
		...
		let total_staked = sp_hamster::p_market::TotalStakingAmount {
			total_staking: 0,
			total_provider_staking: 0,
			total_gateway_staking: 0,
