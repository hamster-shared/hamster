...
fn it_works_for_staking_amount(){
    new_test_pub().execute_with(|| {
        let account_id = 1;
        let price = 10;
        assert_ok!(ResourceOrder::staking_amount(Origin::signed(account_id),price));
...
fn it_works_for_withdraw_amount(){
    new_test_pub().execute_with(|| {
        ...
        assert_noop!(ResourceOrder::withdraw_amount(Origin::signed(1),10),Error::<Test>::StakingNotExist);
        assert_noop!(ResourceOrder::withdraw_amount(Origin::signed(account_id),1000),Error::<Test>::InsufficientStaking);

        assert_ok!(ResourceOrder::withdraw_amount(Origin::signed(account_id),price));
        let amount = ResourceOrder::staking(account_id).unwrap().amount;
        let active_amount = ResourceOrder::staking(account_id).unwrap().active_amount;
        let lock_amount = ResourceOrder::staking(account_id).unwrap().lock_amount;
...
fn it_works_for_create_order_info(){
    new_test_pub().execute_with(|| {
        ...
        assert_noop!(ResourceOrder::create_order_info(Origin::signed(account_id),100,rent_duration,public_key.clone()),Error::<Test>::ResourceNotExist);
        assert_noop!(ResourceOrder::create_order_info(Origin::signed(account_id),resource_index,10000,public_key.clone()),Error::<Test>::ExceedTheRentableTime);
        assert_noop!(ResourceOrder::create_order_info(Origin::signed(account_id),2,rent_duration,public_key.clone()),Error::<Test>::ResourceHasBeenRented);

        assert_ok!(ResourceOrder::create_order_info(Origin::signed(account_id),resource_index,rent_duration,public_key));
...
fn it_works_for_order_exec(){
    new_test_order().execute_with(|| {
        ...
        assert_noop!(ResourceOrder::order_exec(Origin::signed(account_id),100),Error::<Test>::OrderDoesNotExist);
        assert_noop!(ResourceOrder::order_exec(Origin::signed(100),0),Error::<Test>::OrderNotOwnedByYou);
...
fn it_works_for_cancel_order(){
    new_test_order().execute_with(|| {
        ...
        assert_noop!(ResourceOrder::cancel_order(Origin::signed(account_id),100),Error::<Test>::OrderDoesNotExist);
        assert_noop!(ResourceOrder::cancel_order(Origin::signed(100),0),Error::<Test>::OrderNotOwnedByYou);
...
fn it_works_for_heartbeat(){
    new_test_agreement().execute_with(|| {
        ...
        assert_noop!(ResourceOrder::heartbeat(Origin::signed(account_id),100),Error::<Test>::ProtocolDoesNotExist);
        assert_noop!(ResourceOrder::heartbeat(Origin::signed(100),0),Error::<Test>::ProtocolNotOwnedByYou);
        ...
        assert_ok!(ResourceOrder::withdraw_rental_amount(Origin::signed(account_id),0));
        let agreement = ResourceOrder::rental_agreements(0).unwrap();
        assert_eq!(agreement.price,100);
        assert_eq!(agreement.lock_price,51);
...
fn it_works_for_renew_agreement(){
    new_test_agreement().execute_with(|| {
        ...
        assert_noop!(ResourceOrder::renew_agreement(Origin::signed(account_id),100,renew_hour),Error::<Test>::ProtocolDoesNotExist);
        assert_noop!(ResourceOrder::renew_agreement(Origin::signed(account_id),0,999999),Error::<Test>::InsufficientTimeForResource);

        assert_ok!(ResourceOrder::renew_agreement(Origin::signed(account_id),0,renew_hour));
