...
fn it_works_for_default_value() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(Gateway::register_gateway_node(Origin::signed(1), "abcd".as_bytes().to_vec()));
...
fn it_works_heartbeat_logic() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(Gateway::heartbeat(Origin::signed(2), "bcde".as_bytes().to_vec()), Error::<Test>::GatewayNodeNotFound);

        assert_noop!(Gateway::heartbeat(Origin::signed(2), "abcd".as_bytes().to_vec()), Error::<Test>::GatewayNodeNotOwnedByYou);
    });
}
...
fn it_works_heartbeat_value() {
    test_heartbeart_ext().execute_with(|| {
        ...
        let now = 1;
        let gateway_node = Gateway::gateway("abcd".as_bytes().to_vec());
