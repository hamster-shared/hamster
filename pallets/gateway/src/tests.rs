...
fn it_works_for_default_value() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(Gateway::register_gateway_node(Origin::signed(1), "some_peerid".as_bytes().to_vec()));
...
fn it_works_heartbeat_logic() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(Gateway::heartbeat(Origin::signed(1), "some_peerid".as_bytes().to_vec()));

        assert_noop!(Gateway::heartbeat(Origin::signed(2), "another_peerid".as_bytes().to_vec()), Error::<Test>::GatewayNodeNotFound);

        assert_noop!(Gateway::heartbeat(Origin::signed(2), "some_peerid".as_bytes().to_vec()), Error::<Test>::GatewayNodeNotOwnedByYou);
    });
}
